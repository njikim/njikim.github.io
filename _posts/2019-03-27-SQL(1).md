...
-- 단순 sqlplus tool만의 평집 명령어
-- 영구 저장 안됨. sqlplus 실행시마다 해 줘야 함
mysql> set linesize 200
mysql> set pagesize 200
```
...
```sql
-- 급여가 많은 순 (내림차순 - DESC) 으로 이름, 급여, 부서코드를 조회
...
```
```sql
-- 부서별로 급여가 적은 순 (오름차순 - ASC) 으로 이름, 급여, 부서코드 조회
SELECT ename, sal, deptno 
FROM scott_emp
ORDER BY deptno, sal;
```
