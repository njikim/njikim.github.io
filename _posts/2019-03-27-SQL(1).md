...
SELECT ename, deptno, job, max(hiredate) FROM scott_emp;
...
SELECT DAYOFYEAR('2019-12-25')-DAYOFYEAR(NOW());
+------------------------------------------+
...
SELECT ename, hiredate, DATEDIFF(NOW(), hiredate)/7 as totalweek FROM scott_emp;

-- 3. 10번 부서 직원들 중 현재까지의 근무월수를 계산해서 조회
SELECT ename, hiredate, ROUND(DATEDIFF(NOW(), hiredate)/30) as totalmonth FROM scott_emp 
	WHERE deptno=10;
+--------+---------------------+------------+
...
SELECT ename, hiredate, DATE_ADD(hiredate, interval 13 MONTH) as checkingday FROM scott_emp 
	WHERE deptno=20;
+-------+---------------------+---------------------+
...
SELECT ename, hiredate, ROUND(DATEDIFF(NOW(), hiredate)/30) as totalmonth, sal, sal*ROUND(DATEDIFF(NOW(), hiredate)/30) as totalsal FROM scott_emp;
+--------+---------------------+------------+------+----------+
